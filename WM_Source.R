...
df <- data.frame(apparatus2$station, apparatus2$turn_out_seconds)
turnout_average <- data.frame(aggregate(apparatus2$turn_out_seconds ~ apparatus2$station, data=df, mean))
dispatch_count <- data.frame(aggregate(.~apparatus2$station, data=df, FUN=length))


#Combining Stations 0 and 1 into a single station, and removing 98, 15 and 0
dispatch_count[2, 2] = 8490
dispatch_count<- dispatch_count[-c(1, 16, 17), ]
turnout_average <- turnout_average[-c(1, 16, 17), ]
...
dispatch_count$station_full <- sub("^", "Station ", dispatch_count$apparatus2.station.1)
merged_dispatch <- merge(firezone_info, dispatch_count, by.x = "Station", by.y = "station_full")

#Creating a label for the polygons for the dispatch map
merged_dispatch$label <- with(merged_dispatch, paste(
  "<p> <b>","Dispatches =", merged_dispatch$apparatus2.station, 
  "</b> </br>", merged_dispatch$Station,
  "</p>"))
...
dispatch_map <- addPolygons(dispatch_map, data= firezones, stroke=TRUE, color='black', weight=2, opacity= 0.8, fill=TRUE, 
                            fillColor = ~pal_disp(merged_dispatch$apparatus2.station), fillOpacity = 1, popup= (merged_dispatch$label))
...
turnout_map <-  addLegend(turnout_map, position="bottomright", pal= pal_turnout, values= (0:120), title= 'Avg Turnout Time for 2019 (sec)', opacity=1)
turnout_map
